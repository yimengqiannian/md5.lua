...
local function rstrHMACMD5 (key, data)
	...
	ipad[15] = nil
	opad[15] = nil
	if string.len(bkey) > 16 then
	  bkey = binlMD5(bkey, string.len(key) * 8)
	end
	for i = 1,16 do
	  ipad[i] = bit.bxor(bkey[i] , 0x36363636)
	  opad[i] = bit.bxor(bkey[i] , 0x5c5c5c5c)
	end
	hash = binlMD5(concatArray(ipad ,rstr2binl(data)), 512 + string.len(data) * 8)
	return binl2rstr(binlMD5(concatArray(opad,hash), 512 + 128))
end
